...
type LMap = any; // Type for Leaflet map instance
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type LLayerGroup = any; // Type for Leaflet layer group
...
const NetworkMap: React.FC = () => {
  ...
  const nodeLayerRef = useRef<LLayerGroup | null>(null);
  const linkLayerRef = useRef<LLayerGroup | null>(null);
  const pathLayerRef = useRef<LLayerGroup | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch((error) =>
        console.error('Could not load Leaflet library:', error),
      );
  }, []);
  ...
  useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current).setView([12.9797, 77.5907], 13);
    ...
    pathLayerRef.current = L.layerGroup().addTo(map); // Initialize path layer
  }, [leafletLoaded]);
    ...
    ws.current.onmessage = (event) => {
      ...
      switch (data.type) {
        case 'INITIAL_LAYOUT':
          setNodes(data.nodes);
          setNodeStatus(
            data.nodes.reduce((acc, node) => ({ ...acc, [node.id]: 'up' }), {}),
          );
          break;
        case 'COMMUNICATION':
  ...
  useEffect(() => {
    // Draw/update nodes
    if (!mapInstanceRef.current || !nodeLayerRef.current) return;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const L = (window as any).L;
    ...
    if (nodes.length > 0) {
      mapInstanceRef.current.fitBounds(
        L.latLngBounds(nodes.map((n) => n.position)),
        { padding: [50, 50] },
      );
    }
  ...
  useEffect(() => {
    // Draw/update communication links
    if (!mapInstanceRef.current || !linkLayerRef.current) return;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const L = (window as any).L;
    ...
    links.forEach((link) => {
      ...
      if (sourceNode && targetNode) {
        const polyline = L.polyline(
          ...
          {
            color: linkColors[link.type] || linkColors.default,
            weight: 2.5,
            opacity: 0.8,
          },
        );
  ...
  useEffect(() => {
    // Draw/update the active path highlight
