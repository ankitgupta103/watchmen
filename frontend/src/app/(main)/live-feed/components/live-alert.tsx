...
interface CriticalAlert {
  ...
  eventstr?: string; // For MQTT events
}
...
export default function CriticalAlertSystem({
  ...
  useEffect(() => {
    const handleMqttEvent = (event: CustomEvent) => {
      const { machineId, event: mqttEvent } = event.detail;
      
      // Create alert from MQTT event
      const alert: CriticalAlert = {
        id: `mqtt-${mqttEvent.id}`,
        timestamp: new Date(mqttEvent.timestamp),
        machineId: machineId,
        machineName: `Machine-${machineId}`,
        type: 'mqtt_event',
        severity: 'critical',
        message: `MQTT Event: ${mqttEvent.eventstr}`,
        location: { lat: 12.9716, lng: 77.5946 }, // Default location, you might want to get real location
        acknowledged: false,
        autoGenerated: true,
        eventstr: mqttEvent.eventstr,
      };
      
      addAlert(alert);
    };

    window.addEventListener('mqtt-event-received', handleMqttEvent as EventListener);
    
    return () => {
      window.removeEventListener('mqtt-event-received', handleMqttEvent as EventListener);
    };
  ...
  const addAlert = useCallback(
    async (alert: CriticalAlert) => {
      ...
      // Enhanced toast for MQTT events
      toast.custom(
        (t) => (
          <div
            className={cn(
              ...
              { 'border-blue-400 bg-blue-50': alert.type === 'mqtt_event' },
            )}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900">
                  {alert.type === 'mqtt_event' ? 'MQTT EVENT' : alert.type.replace('_', ' ').toUpperCase()}
                </p>
                <p className="text-sm text-gray-700">
                  {alert.machineName}: {alert.eventstr || alert.message}
                </p>
              </div>
            </div>
            <Button
                    ...
                    {alerts.map((alert) => (
                      <div
                        ...
                        className={cn(
                          ...
                          alert.type === 'mqtt_event' && !alert.acknowledged && 'border-blue-200 bg-blue-50'
                        )}
                      >
