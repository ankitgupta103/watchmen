...
class VyomClient:
    ...
    def on_image_arrive(
        self, node_hn: str, image: bytes, timestamp: str = None, filename: str = None, severity: Union[int,str] = 1
    ):
        """_summary_
        Args:
    ...
    def on_image_bb_arrive(
        self, node_hn: str, image: bytes, timestamp: str = None, filename: str = None
    ):  # LIVE with priority 2
        """_summary_
    ...
    def on_event_arrive(
        self, node_hn: str, timestamp: str = None, filename: str = None
    ):
        """_summary_
        ...
        try:
            ...
            payload = {
                "image_key": f"{file_s3_dir}/{filename}.jpg",
                "image_crop_key": f"{file_s3_dir}/{filename}.jpg",
            }
            self.writer.write_message(
    ...
    def on_hb_arrive(
        self, node_hn: str, timestamp: str = None, lat: int = None, long: int = None
    ):
        """
...
if __name__ == "__main__":
    ...
    try:
        ...
        # test on_image_bb_arrive
        image_url = "https://sample-videos.com/img/Sample-jpg-image-50kb.jpg"
        response = requests.get(image_url)
        if response.status_code == 200:
            binary_data = response.content
            client.on_image_bb_arrive(
                node_hn="rpi2", image=binary_data, filename="test_bb.jpg"
            )
        else:
            print(
                f"[Error] Failed to download {image_url} (Status: {response.status_code})"
        ...
        client.on_event_arrive(node_hn="rpi3", filename="test_bb.jpg")
