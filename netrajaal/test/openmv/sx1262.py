...
def wait_for_interrupt(timeout_ms=60000, interrupt_type='any'):
    """
    Wait for interrupt (polling both DIO1 pin and interrupt flags).
    
    This function polls the DIO1 pin directly (like wait_for_tx_done/wait_for_rx_done)
    and also checks the interrupt flag. When an interrupt is detected, it reads
    the IRQ status to determine which specific interrupt occurred and sets the
    appropriate flags.
    ...
    while True:
        ...
        # Check if interrupt occurred (either via callback flag or by polling DIO1 pin)
        interrupt_detected = False
        
        # Check interrupt flag (set by callback)
        if interrupt_flag_occurred:
            interrupt_detected = True
            interrupt_flag_occurred = False  # Clear the flag
        
        # Also poll DIO1 pin directly (fallback in case callback doesn't work)
        if DIO1_PIN.value() == 1:
            interrupt_detected = True
        
        # If interrupt detected, read IRQ status to determine which interrupt occurred
        if interrupt_detected:
            # Read IRQ status (SPI communication - done in main loop, not interrupt handler)
            irq_status = get_irq_status()
            if irq_status is not None:
                ...
                if interrupt_type == 'any':
                    ...
                    return flags
                # If interrupt occurred but not the requested type, continue waiting
                # (might be a different interrupt type)
