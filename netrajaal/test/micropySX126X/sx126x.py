...
class SX126X:
    ...
    def __init__(self, spi_bus, clk, mosi, miso, cs, irq, rst, gpio, spi_baudrate=2000000, spi_polarity=0, spi_phase=0):
        self._irq = irq
        if implementation.name == 'micropython':
          # Try OpenMV RT1062 format first (without pin parameters in SPI constructor)
          try:
              # OpenMV RT1062 format: SPI(bus, baudrate=..., polarity=..., phase=..., bits=8, firstbit=SPI.MSB)
              # Pins are pre-configured for the SPI bus, not passed as parameters
              self.spi = SPI(spi_bus, baudrate=spi_baudrate, polarity=spi_polarity, phase=spi_phase,
                           bits=8, firstbit=SPI.MSB)
              self._spi_type = 'openmv'
          except:
              try:
                  ...
                  self.spi.init(baudrate=spi_baudrate, polarity=spi_polarity, phase=spi_phase,
                              bits=8, firstbit=SPI.MSB)
                  self._spi_type = 'openmv_init'
              except:
                  try:
                      # Pycom variant uPy
                      self.spi = SPI(spi_bus, mode=SPI.MASTER, baudrate=spi_baudrate, pins=(clk, mosi, miso))
                      self._spi_type = 'pycom'
                  except:
                      # Generic variant uPy
                      self.spi = SPI(spi_bus, baudrate=spi_baudrate, sck=Pin(clk), mosi=Pin(mosi), miso=Pin(miso))
                      self._spi_type = 'generic'
