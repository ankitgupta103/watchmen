...
def main():
    ...
    while True:
        ...
        try:
            ...
            # Save raw image
            # CRITICAL: Get image size BEFORE saving (don't access img.bytearray() after save)
            img_size_before_save = len(img.bytearray())
            raw_path = None
            try:
                raw_path = f"{MY_IMAGE_DIR}/{my_addr}_{event_epoch_ms}_raw.jpg"
                print(f"DEBUG, [CAPTURE] Saving raw image to {raw_path} : imbytesize = {img_size_before_save}")
                # Save image - img object must stay alive until save completes
                img.save(raw_path)
                # CRITICAL: Keep img object alive and give time for write buffer to flush
                utime.sleep_ms(300)  # Allow internal buffer to flush
                os.sync()  # Force filesystem sync to SD card
                utime.sleep_ms(500)  # Additional delay after sync for SD card to finish writing
            ...
            try:
                ...
                # Give time for file write to complete before syncing
                utime.sleep_ms(300)  # Allow internal buffer to flush
                os.sync()  # Force filesystem sync to SD card
                utime.sleep_ms(500)  # Additional delay after sync for SD card to finish writing
        ...
        print(f"DEBUG, [CAPTURE] Waiting {SLEEP_TIME} second before next capture...")
