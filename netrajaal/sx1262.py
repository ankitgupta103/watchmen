...
class sx126x:
    ...
    def __init__(
        ...
        if freq > FREQ_RANGE_900MHZ_START:
            ...
            raise ValueError(
                f"Frequency {freq} MHz out of valid range (410-493 or 850-930)"
            )
    ...
    def set(
        ...
        if air_speed_temp is None:
            raise ValueError(
                f"Invalid air speed: {air_speed}. Valid: {list(self.lora_air_speed_dic.keys())}"
            )

        buffer_size_temp = self.lora_buffer_size_dic.get(buffer_size, None)
        if buffer_size_temp is None:
            raise ValueError(
                f"Invalid buffer size: {buffer_size}. Valid: {list(self.lora_buffer_size_dic.keys())}"
            )

        power_temp = self.lora_power_dic.get(power, None)
        if power_temp is None:
            raise ValueError(
                f"Invalid power: {power} dBm. Valid: {list(self.lora_power_dic.keys())}"
            )
        ...
        if not relay:
            ...
            self.cfg_reg[REG_ADDR_UART_AIR] = (
                self.SX126X_UART_BAUDRATE_115200 + air_speed_temp
            )
        ...
        for attempt in range(CFG_RETRY_ATTEMPTS):
            ...
            if self.ser.any():
                ...
                if r_buff and len(r_buff) > 0 and r_buff[0] == RESPONSE_SUCCESS:
                    ...
                    if received_config:
                        self._display_config_table(
                            "CONFIGURATION RECEIVED (CONFIRMED)", received_config
                        )
                    else:
                    ...
                    if received_config and sent_config:
                        ...
                        if received_config["node_addr"] != sent_config["node_addr"]:
                            config_match = False
                            mismatches.append(
                                f"Node Address: sent={sent_config['node_addr']}, received={received_config['node_addr']}"
                            )
                        if received_config["frequency"] != sent_config["frequency"]:
                            config_match = False
                            mismatches.append(
                                f"Frequency: sent={sent_config['frequency']}, received={received_config['frequency']}"
                            )
                        if received_config["uart_baud"] != sent_config["uart_baud"]:
                            config_match = False
                            mismatches.append(
                                f"UART Baud: sent={sent_config['uart_baud']}, received={received_config['uart_baud']}"
                            )
                        if received_config["air_speed"] != sent_config["air_speed"]:
                            config_match = False
                            mismatches.append(
                                f"Air Speed: sent={sent_config['air_speed']}, received={received_config['air_speed']}"
                            )
                        if received_config["power"] != sent_config["power"]:
                            config_match = False
                            mismatches.append(
                                f"TX Power: sent={sent_config['power']}, received={received_config['power']}"
                            )

                        if config_match:
                            logger.info(
                                f"===> CONFIGURATION SUCCESSFUL, all parameters matched! <===\n"
                            )
                        else:
                            logger.warning(
                                f"===> CONFIGURATION PARTIALLY SUCCESSFUL <==="
                            )
                    ...
                    logger.warning(
                        f"Configuration failed, unexpected response: {[hex(x) for x in r_buff] if r_buff else 'None'}"
                    )
            else:
            ...
            if attempt == CFG_RETRY_ATTEMPTS - 1:
                ...
                logger.error(
                    f"  Attempted {CFG_RETRY_ATTEMPTS} times with no valid response"
                )
    ...
    def _parse_config_bytes(self, cfg_bytes):
        ...
        cfg_persistent = (
            "Persistent (0xC0)"
            if cfg_header == CFG_HEADER_PERSISTENT
            else "Volatile (0xC2)"
        )
    ...
    def _display_config_table(self, title, config_dict):
        ...
        logger.info(
            f"  Node Address         : {config_dict['node_addr']} (0x{config_dict['node_addr']:04X})"
        )
        ...
        logger.info(
            f"  Encryption Key       : {config_dict['crypt_key']} (0x{config_dict['crypt_key']:04X})"
        )
        logger.info(
            f"  Packet RSSI          : {'Enabled' if config_dict['rssi_enabled'] else 'Disabled'}"
        )
        logger.info(
            f"  Noise RSSI           : {'Enabled' if config_dict['noise_rssi_enabled'] else 'Disabled'}"
        )
        logger.info(
            f"  Raw Bytes            : {[hex(x) for x in config_dict['raw_bytes']]}\n"
        )
    ...
    def get_settings(self):
        ...
        if (
            not response
            or len(response) < 12
            or response[0] != RESPONSE_SUCCESS
            or response[2] != 0x09
        ):
            logger.warning(
                f"Invalid response: {[hex(x) for x in response] if response else 'None'}"
            )
            self.M1.value(0)  # Exit config mode
            return
        ...
        logger.info(
            f"Noise RSSI      : {'Enabled' if noise_rssi_enabled else 'Disabled'}"
        )
        logger.info(f"=" * 40)
    ...
    def send(self, target_addr, message):
        ...
        offset_frequency = self.freq - (
            FREQ_RANGE_900MHZ_START
            if self.freq > FREQ_RANGE_900MHZ_START
            else FREQ_RANGE_400MHZ_START
        )
    ...
    def receive(self):
        ...
        if self.ser.any():
            ...
            # Read message - try readline first, but also check for complete message
            r_buff = self.ser.readline()
            
            # If readline() didn't get newline, try reading more data
            # This handles cases where message arrives in chunks or is incomplete
            if r_buff and len(r_buff) > 0 and r_buff[-1] != 0x0A:
                # Wait a bit more and check if more data is available
                time.sleep_ms(100)  # Increased wait for complete message
                if self.ser.any():
                    # Try readline again to get the rest
                    additional = self.ser.readline()
                    if additional:
                        r_buff = r_buff + additional
                    else:
                        # If readline doesn't work, read available bytes directly
                        available = self.ser.any()
                        if available > 0:
                            additional = self.ser.read(available)
                            if additional:
                                r_buff = r_buff + additional
                                # Look for newline in the combined buffer
                                newline_pos = r_buff.find(b'\n')
                                if newline_pos >= 0:
                                    r_buff = r_buff[:newline_pos+1]
                                # If still no newline, wait one more time for very long messages
                                elif self.ser.any():
                                    time.sleep_ms(50)
                                    more_data = self.ser.read(self.ser.any())
                                    if more_data:
                                        r_buff = r_buff + more_data
                                        newline_pos = r_buff.find(b'\n')
                                        if newline_pos >= 0:
                                            r_buff = r_buff[:newline_pos+1]
    ...
    def get_channel_rssi(self):
        ...
        if self.ser.any():
            ...
            if (
                re_temp
                and len(re_temp) >= 4
                and re_temp[0] == RSSI_RESPONSE_HEADER[0]
                and re_temp[1] == RSSI_RESPONSE_HEADER[1]
                and re_temp[2] == RSSI_RESPONSE_HEADER[2]
            ):
