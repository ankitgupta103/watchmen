...
# Global variables - should be set from main.py before logger instance is created
# If not set, will use default values as fallback
...
# Lazy logger wrapper - initializes logger instance on first access
class LazyLogger:
    """Lazy logger that initializes only when first accessed"""
    _logger_instance = None
    
    def _get_logger(self):
        """Get or create logger instance, checking global variables with fallback"""
        if self._logger_instance is None:
            # Check if global variables are set, use defaults if None
            save_log = SAVE_LOG if SAVE_LOG is not None else True
            log_level = LOG_LEVEL if LOG_LEVEL is not None else "INFO"
            self._logger_instance = setup_logger(show_terminal=True, log_level=log_level, save_log=save_log)
        return self._logger_instance
    
    def __getattr__(self, name):
        """Forward all attribute access to the actual logger instance"""
        return getattr(self._get_logger(), name)

# Initialize the lazy logger instance
logger = LazyLogger()
