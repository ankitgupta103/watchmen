...
class SimpleLogger:
    ...
    def _new_(cls, *args, **kwargs):
        ...
        if cls._instance is None:
            print("INFO - Singleton Logger class Initiated, _new_")
            cls.instance = super(SimpleLogger, cls).__new_(cls)
        else:
            # print("DEBUG - Singleton Logger class already instantiated, _new_")
            pass
    ...
    def _open_log_file(self):
        ...
        try:
            ...
            if clean_log_dir.endswith("/"):
                log_file_path = clean_log_dir + self.log_file_name
            else:
                log_file_path = clean_log_dir + "/" + self.log_file_name
    ...
    def _defelete_and_recreate_directory(self, log_dir, log_file_name):
        """Delete existing log directory and recreate log file"""
        try:
            # Delete existing log directory and its contents
            if log_dir in os.listdir(os.path.dirname(log_dir)):
                ...
                os.rmdir(log_dir)
                print(f"INFO - Deleted existing log directory: {log_dir}")
        except OSError as e:
            print(f"WARNING - Failed to delete log directory {log_dir}: {e}")
