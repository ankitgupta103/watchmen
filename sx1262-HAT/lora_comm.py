...
class RFComm:
    ...
    def _process_read_message(self, msgstr):
        ...
        if msgid is None or len(msgid) < 3:
            print(f" ERROR reading msgid is none or too short in MSG: {msgstr}")
            return
    ...
    def _send_broadcast(self, payload, mst):
        ...
        return self._actual_send(msgstr, 65535)
    ...
    def _send_unicast(self, payload, mst, dest, wait_for_ack = True, retry_count = 3, override_idstr = None):
        ...
        msgstr = f"{msgid};{payload}"
        if not wait_for_ack:
            return self._actual_send(msgstr, dest)
        ...
        for i in range(retry_count):
            ...
            sent_succ = self._send_with_retries(msgstr, msgid, dest)
        return sent_succ
